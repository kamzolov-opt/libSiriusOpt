...
Indicies = np.array([[1, 0]])


def func_error(x):
    # params is consist of [a, b]
    ...
    for i in Indicies:
        xi = X[i, :]
        yi = Y[i]
        fwd = forwardEvalute(xi, a, b)
        results += ((fwd.Fhat - yi) ** 2) / 2
    return results / len(Indicies)
...
def grad_func(x):
    ...
    gradient = None
    for i in Indicies:
        fwd = forwardEvalute(np.concatenate((x, np.array([1.0]))), X[i], Y[i])
        if gradient is None:
            gradient = np.asarray([partial_DL_da_compute(fwd, y), partial_DL_db_compute(fwd, y)])
        else:
            gradient += np.asarray([partial_DL_da_compute(fwd, y), partial_DL_db_compute(fwd, y)])
    return gradient / len(Indicies)
...
for i in range(experiments):
    ...
    a = makeRandom(
        a)  # Values of a[i,j] where a[i,j] is the weight of the edge connected i-th input unit with j-th hidden unit
    ...
    predicateCheck2 = np.linalg.norm(
        np.reshape(partial_DL_da(fwd, y), (1, n * m)) - np.reshape(partial_DL_da_compute(fwd, y), (1, n * m))) < 0.001
